              ...
              {experience.map(exp => (
                <div key={exp.id}>
                  <div className="flex justify-between items-start mb-0.5">
                    ...
                    <span className="font-medium text-[12px] text-slate-600 whitespace-nowrap">
                      {exp.startDate} — {exp.current ? 'Present' : exp.endDate}
                    </span>
                  </div>
                  <div className="text-[13px] text-indigo-700 font-medium mb-1.5">{exp.company}</div>
            ...
            <div className="space-y-4">
              {education.map(edu => (
                <div key={edu.id}>
                  ...
                  {edu.description && (
                    <div className="text-[12px] mt-1 text-slate-600 whitespace-pre-wrap">{edu.description}</div>
                  )}
                </div>
              ))}
            </div>
              ...
              {projects.map(proj => (
                <div key={proj.id}>
                  <div className="flex items-baseline mb-1">
                    ...
                    {proj.technologies && <span className="text-[12px] text-indigo-700 font-medium ml-2 border-l border-slate-300 pl-2">{proj.technologies}</span>}
                    {proj.url && <a href={proj.url} className="text-[11px] ml-auto text-blue-600 hover:underline" target="_blank" rel="noreferrer">View Details</a>}
                  </div>
            ...
            <div className="grid grid-cols-2 gap-x-6 gap-y-2">
              {skills.map(skill => (
                <div key={skill.id} className="text-[13px]">
                  <span className="font-bold text-slate-800 mr-2">{skill.category}:</span>
          ...
          <div key="certifications" className="mb-6">
            ...
            <div className="space-y-2">
              {certifications.map(cert => (
                <div key={cert.id} className="flex justify-between items-center text-[13px]">
                  <div>
                    <span className="font-bold text-slate-900">{cert.title}</span>
                    {cert.issuer && <span className="text-slate-600"> — {cert.issuer}</span>}
                    {cert.url && <a href={cert.url} className="ml-2 text-[11px] text-blue-600 hover:underline" target="_blank" rel="noreferrer">[View]</a>}
                  </div>
            ...
            <div className="space-y-3">
              {achievements.map(ach => (
                <div key={ach.id}>
                  ...
                  {ach.description && <div className="text-[13px] text-slate-700 whitespace-pre-wrap">{ach.description}</div>}
                </div>
              ))}
            </div>
          </div>
            ...
            <div className="flex flex-wrap gap-x-8 gap-y-2">
              {languages.map(lang => (
                <div key={lang.id} className="text-[13px]">
                  ...
                  <span className="text-slate-600 ml-2 border-l border-slate-300 pl-2">{lang.proficiency}</span>
                </div>
              ))}
            </div>
          </div>
            ...
            <div className="grid grid-cols-2 gap-6">
              {references.map(ref => (
                <div key={ref.id} className="text-[13px]">
                  ...
                  <div className="text-indigo-700 font-medium">{ref.position}{ref.company && `, ${ref.company}`}</div>
