...
import { Upload, FileText, X, Loader2, CheckCircle, XCircle, Target, GraduationCap, BookOpen, Zap, Clock, Bookmark, ArrowLeft, Sparkles } from 'lucide-react'
      ...
      <div className="max-w-4xl mx-auto space-y-6">
        ...
        {!result && (
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <Card>
              <CardContent className="p-6">
                <label htmlFor="targetRole" className="block text-sm font-medium text-[var(--text-primary)] mb-2">
                ...
                />
                <p className="mt-1 text-xs text-[var(--text-tertiary)]">Enter the role you want to transition into for accurate gap analysis.</p>
              </CardContent>
            </Card>
            ...
            <div className="flex items-center gap-3">
              <Button type="submit" disabled={!file || !targetRole.trim() || isPending} size="lg">
                <Sparkles className="h-4 w-4" /> {isPending ? 'Analyzing...' : 'Analyze Skill Gap'}
              </Button>
            </div>
            ...
            <Card>
              <CardContent className="p-6">
                <div className="flex items-center gap-2 mb-3">
                  <GraduationCap className="h-5 w-5 text-[var(--color-primary-500)]" />
                  <h2 className="font-semibold text-[var(--text-primary)]">Gap Analysis</h2>
                </div>
                <p className="text-sm text-[var(--text-secondary)] leading-relaxed whitespace-pre-wrap">{result.gapAnalysis}</p>
              </CardContent>
            </Card>
                ...
                <Card>
                  <CardContent className="p-5">
                    <div className="flex items-center gap-2 mb-3">
                      ...
                      <Badge variant="success" size="xs">{result.currentSkills.length}</Badge>
                    </div>
                ...
                <Card>
                  <CardContent className="p-5">
                    <div className="flex items-center gap-2 mb-3">
                      ...
                      <Badge variant="danger" size="xs">{result.missingSkills.length}</Badge>
                    </div>
                    <div className="flex flex-wrap gap-2">
                      ...
                      <div key={i} className="rounded-xl border border-[var(--border-color)] bg-[var(--bg-secondary)] p-5">
                        <div className="flex items-start justify-between gap-4 mb-3">
                          <div>
                            ...
                            {phase.focus && <p className="mt-1 text-xs text-[var(--text-tertiary)]">{phase.focus}</p>}
                          </div>
